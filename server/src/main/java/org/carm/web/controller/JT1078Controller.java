...
import org.carm.protocol.t1078.*;
...
import org.carm.protocol.jsatl12.T9208;
...
public class JT1078Controller {
    ...
    public Mono<R<T1003>> T9003(@RequestBody JTMessage request) {
        return messageService.requestR(request.setMessageId(JT1078.查询终端音视频属性), T1003.class);
    }
    ...
    public Mono<R<T1205>> T9201(@RequestBody T9201 request) {
        return messageService.requestR(request, T1205.class);
    }
    ...
    public Mono<R<T0001>> T9302(@RequestBody T9302 request) {
        return messageService.requestR(request.setMessageId(JT1078.云台调整焦距控制), T0001.class);
    }

    @Operation(summary = "9303 云台调整光圈控制")
    @PostMapping("9303")
    public Mono<R<T0001>> T9303(@RequestBody T9302 request) {
        return messageService.requestR(request.setMessageId(JT1078.云台调整光圈控制), T0001.class);
    }

    @Operation(summary = "9304 云台雨刷控制")
    @PostMapping("9304")
    public Mono<R<T0001>> T9304(@RequestBody T9302 request) {
        return messageService.requestR(request.setMessageId(JT1078.云台雨刷控制), T0001.class);
    }

    @Operation(summary = "9305 红外补光控制")
    @PostMapping("9305")
    public Mono<R<T0001>> T9305(@RequestBody T9302 request) {
        return messageService.requestR(request.setMessageId(JT1078.红外补光控制), T0001.class);
    }

    @Operation(summary = "9306 云台变倍控制")
    @PostMapping("9306")
    public Mono<R<T0001>> T9306(@RequestBody T9302 request) {
        return messageService.requestR(request.setMessageId(JT1078.云台变倍控制), T0001.class);
    }
}